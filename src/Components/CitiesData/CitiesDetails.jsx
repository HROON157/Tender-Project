...
const CitiesDetails = () => {
  ...
  useEffect(() => {
    const fetchTendersByCity = async () => {
      try {
        ...
        const sortedCities = tenderData.sort((a, b) => b.id - a.id);
        
        // Filter tenders based on the current city name
        const filteredTenders = sortedCities.filter(tender => tender.cityName === name);
  ...
  // Search and pagination on tenders based on title
